...
@Component({
  selector: 'app-add-lcomm',
  templateUrl: './add-lcomm.component.html',
  styleUrls: ['./add-lcomm.component.css']
})
export class AddLcommComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    {
     this.panierService.formData =this.fb.group(Object.assign({},this.panierService.list[this.data.lcommandeIndex]));
    }
   this.articleService.getAll().subscribe(
      response =>{this.articleList.push= response;}
     );
}
...
{
  this.panierService.list[this.data.lcommandeIndex] = this.panierService.formData.value;
}
this.dialogRef.close();
}
